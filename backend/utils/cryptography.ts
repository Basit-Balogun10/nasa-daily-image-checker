...
export const encryptData = (data: string) => {
    const cipher = crypto.createCipheriv(algorithm, Buffer.from(secretKey, 'hex'), Buffer.from(initVector, 'hex'))
...
export const decryptData = (encryptedData: string) => {
    const decipher = crypto.createDecipheriv(algorithm, Buffer.from(secretKey, 'hex'), Buffer.from(initVector, 'hex'))
    const decryptedData =
